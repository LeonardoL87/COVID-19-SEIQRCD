function [S,E,I,Q,R,D,P] = SEIQRDP2(alpha,beta,gamma,delta,lambda0,kappa0,Npop,T0,E0,I0,Q0,R0,D0,P0,t)
...
for i=1:N
    if i>T0/dt
        T(i)=0.9;
%         T(i)=alpha(1)+alpha(1)*.7; %fig1 opcio 2
%         T(i)=alpha(1)+alpha(1)*.5; %fig1 opcio 3 y fig3 World
%         T(i)=alpha(1)+alpha(1)*.3; %fig1 y Fig3 World
%         T(i)=alpha(1)+alpha(1)*.1; %fig2 
        kappa(i)=kappa(i)*alpha(i); %fig1 y 2 y Fig3 World
        alpha(i)=alpha(i)*1/10;%fig1 y 2 y Fig3 World
...
for ii=1:N-1
    A = getA(alpha(ii),gamma,delta,lambda(ii),kappa(ii),T(ii));
        Y(3,ii)=Y(3,ii);
        SI = Y(1,ii)*Y(3,ii);
    F = zeros(7,1);
...
% plot(T,I)
    ...
    function [A] = getA(alpha,gamma,delta,lambda,kappa,T)
        A = zeros(7);
        % S
        A(1,1:7) = [-alpha, 0, 0, 0, 0, 0, T];
%         A(1,7) = T;
        % E
        A(2,2) = -gamma;
        % I
